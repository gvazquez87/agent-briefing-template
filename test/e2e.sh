...
# A clone only carries committed state; bring over uncommitted changes so the
# test always exercises the working tree being edited, not stale HEAD.
...
sed -i 's|^briefing-hub: .*|briefing-hub: /some/other/hub|' "$P/.cursor/rules/briefing.mdc"
